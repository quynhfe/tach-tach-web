// ---------------------------------------------------------------------------
// Style bản đồ "cream/brand" — BẢN SAO của `src/lib/mapStyle.ts` bên repo app.
// Hai repo khác toolchain nên không import chéo được; sửa màu bản đồ thì phải
// sửa cả hai chỗ, README đã ghi cặp phụ thuộc này.
// Đây là style-spec chuẩn nên Mapbox GL JS đọc y như Mapbox GL Native của app.
// ---------------------------------------------------------------------------
...
/** Style object cho `new mapboxgl.Map({ style })`. */
export const BRAND_MAP_STYLE = brandStyle as unknown as import("mapbox-gl").StyleSpecification;

/** Style URL riêng (Mapbox Studio) nếu có — không thì dùng style cream tự chứa. */
export const MAPBOX_STYLE_URL = process.env.NEXT_PUBLIC_MAPBOX_STYLE_URL || undefined;
